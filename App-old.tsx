...
const GasStationView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const fetchNearbyStations = async (lat: number, lng: number) => {
    try {
      ...
      setError('실제 주유소 데이터를 가져올 수 없어 샘플 데이터를 표시합니다.');
      
    } catch (err) {
      console.error('주유소 정보 로드 오류:', err);
      setError('주유소 정보를 불러오는 중 오류가 발생했습니다.');
...
const WeatherView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const fetchWeatherData = async (lat: number, lng: number) => {
    try {
      ...
      // 더미 데이터로 대체 (API 호출 실패시)
      setWeather({
        carWash: {
          level: '좋음',
          comment: '오늘 세차하기 좋은 날입니다!'
        },
        airQuality: {
          pm10: '30',
          pm25: '15',
...
const HealthView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const fetchHealthData = async () => {
    try {
      ...
      const isCold = currentHour < 10 || currentHour > 18;
  ...
  const fetchParkingLots = async () => {
    try {
      ...
      const dummyParkingLots: ParkingLot[] = [
        {
          prkplceNo: '1',
          prkplceNm: `${selectedRegion} 시청 공영주차장`,
          prkcmprt: '100',
          curPrkCnt: '45',
          address: `${selectedRegion}특별시 중구`,
          latitude: '37.5665',
          longitude: '126.9780'
        },
        {
          prkplceNo: '2',
          prkplceNm: `${selectedRegion} 역 공영주차장`,
          prkcmprt: '150',
          curPrkCnt: '120',
          address: `${selectedRegion}특별시 강남구`,
          latitude: '37.5665',
          longitude: '126.9780'
        },
        {
          prkplceNo: '3',
          prkplceNm: `${selectedRegion} 문화센터 주차장`,
          prkcmprt: '80',
          curPrkCnt: '20',
          address: `${selectedRegion}특별시 송파구`,
          latitude: '37.5665',
          longitude: '126.9780'
        },
        {
          prkplceNo: '4',
          prkplceNm: `${selectedRegion} 공원 주차장`,
          prkcmprt: '60',
          curPrkCnt: '55',
          address: `${selectedRegion}특별시 마포구`,
          latitude: '37.5665',
          longitude: '126.9780'
        },
        {
          prkplceNo: '5',
          prkplceNm: `${selectedRegion} 종합운동장 주차장`,
          prkcmprt: '200',
          curPrkCnt: '80',
          address: `${selectedRegion}특별시 영등포구`,
          latitude: '37.5665',
          longitude: '126.9780'
        }
      ];
      ...
      console.error('주차장 정보 로드 오류:', err);
      
      // 오류시에도 기본 데이터 표시
      setParkingLots([
        {
          prkplceNo: '1',
          prkplceNm: `${selectedRegion} 주차장`,
          prkcmprt: '50',
          curPrkCnt: '20',
